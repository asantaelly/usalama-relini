...
<div class="card shadow mb-4 col-lg-12">
  ...
  <div class="card-body">
		<form action="{{route('progress.update', ['progress' => $progress->id])}}" method="POST">
			@csrf
			@method('PUT')
			<div class="row mb-3">
				<div class="form-group col-lg-4">
					...
					<div>
						<select id="accident_id" name="accident_id" required="required" class="custom-select @error('accident_id') is-invalid @enderror">
							<option value=""  disabled>Select Accident</option>
							@foreach ($accidents as $accident)
							<option value="{{$accident['value']}}" @if ($progress->accident_id == $accident['value'])
								Selected @endif>{{$accident['option']}}</option>
							@endforeach
						</select>
					</div>
					@error('accident_id')
			...
			<div class="row input_fields_wrap_progress">
				...
				<textarea id="particulars" name="particulars[0][particulars]" cols="40" rows="5"  class="form-control @error('particulars') is-invalid @enderror">{{$progress->particulars}}</textarea>
					@error('particulars')
					<span class="invalid-feedback" role="alert">
							<strong>{{ $message }}</strong>
						</span>
