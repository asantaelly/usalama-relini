...
<div class="card shadow mb-4 col-lg-12">
  ...
  <div class="card-body">
		<form action="{{route('officer.update', ['officer' => $officer->id])}}" method="POST">
			@csrf
			@method('PUT')
			<div class="row" >
				<div class="form-group col-lg-4">
					<label for="title">Title</label> 
					<input id="title" name="title" placeholder="Title" type="text" value="{{$officer->title}}"  required="required" class="form-control @error('title') is-invalid @enderror">
					@error('title')
				...
				<div class="form-group col-lg-4">
					...
					<input id="name" name="name" placeholder="Name" type="text" value="{{$officer->name}}" class="form-control @error('name') is-invalid @enderror" required="required">
					@error('name')
						<span class="invalid-feedback" role="alert">
							<strong>{{ $message }}</strong>
						</span>
			...
			<div class="row">
				<div class="form-group col-lg-4">
					...
					<input id="ext_no" name="ext_no" placeholder="EXT NO." type="text" value="{{$officer->ext_no}}" class="form-control @error('ext_no') is-invalid @enderror" required="required">
					@error('ext_no')
					<span class="invalid-feedback" role="alert">
								<strong>{{ $message }}</strong>
						</span>
				...
				<div class="row input_fields_wrap_contact">
					@if (!isset($officer->officer_contacts[0]->id))
					<div class="form-group col-lg-4">
					...
					<div class="row col-lg-12">
						<div class="form-group col-lg-4 @if ($loop->first)
							
						@else
								added-{{$loop->index}}
						@endif">
							<label for="contact">Contact</label> 
						<input id="contact" name="contact[{{$loop->index}}][phone_no]" value="{{$contact->phone_no}}" placeholder="+255768564536"  type="text" class="form-control @error('contact') is-invalid @enderror">
							@error('contact')
							<span class="invalid-feedback" role="alert">
										<strong>{{ $message }}</strong>
								</span>
